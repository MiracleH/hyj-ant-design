...
export default class BaseList extends PureComponent {
  ...
  getColumns = (tableTitle) => {
    let arr = [];
    tableTitle && tableTitle.length && tableTitle.forEach(val => {
        let obj = {};
        obj.title = val.name;
    ...
    if(this.props.handleOption.show) {
        ...
        handleObj.render = (dataIndex, record) => {
            return (
                <Fragment>
                    {
                        this.props.handleOption.data.map((hVal,index) => {
                            if(hVal.url && this.props.handleOption.data.length != (index + 1)) {
                                return (
                                    <Fragment key={index}>
                                        <Link key={index} to={`${hVal.url}/${dataIndex}`}>{hVal.name}</Link>
                                        <Divider type="vertical" />
                                    </Fragment>
                                )
                            }else if(hVal.todo && hVal.url && this.props.handleOption.data.length != (index + 1)) {
                                return (
                                    <Fragment key={index}>
                                        <a href='javascript:void(0);' key={index}  onClick={hVal.todo.bind(this,dataIndex, this.searchLists)}>删除</a>
                                        <Divider type="vertical" />
                                    </Fragment>
                                )
                            }else if(hVal.url) {
            ...
            <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
                {
                    this.props.formList.map((value, index) => {
                        return <Col md={8} sm={12} key={index}>
            ...
            <div style={{ overflow: 'hidden' }}>
                <span style={{ float: 'right', marginBottom: 24 }}>
                    ...
                    <Button style={{ marginLeft: 8 }} onClick={this.handleFormReset}>重置</Button>
                    {
                        ...
                        <Link to={this.props.handleOption.add.url}><Button style={{ marginLeft: 8 }} type="primary">{this.props.handleOption.add.btnName}</Button></Link>
                        : null
                    }
                </span>
            </div>
  ...
  render() {
    const {
        data, pagination, tableTitle, loading
    } = this.props;
